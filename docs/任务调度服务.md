...
```shell
curl 'http://host.docker.internal:3030/api/agenda/list?access_token=<access_token>' -H 'content-type: application/json' -d '{"name":"job001","finished":false}'
```

被禁用的任务。

```shell
curl 'http://host.docker.internal:3030/api/agenda/list?access_token=<access_token>' -H 'content-type: application/json' -d '{"name":"job001","disabled:true}'
```
