...
用`access_token`做参数，调用`/auth/client`接口获得用户信息。
...
```bash
curl "http://localhost:3070/auth/captcha?appid=abc&captchaid=123&returnType=text"
```
...
```bash
curl -X POST -H "Content-type: application/json" "http://localhost:3070/auth/authenticate" -d '{"username":"admin","password":"admin","code":"bp7k","appid":"tmw","captchaid":"abc"}'
```
...
```bash
curl "http://localhost:3001/auth/client?access_token=xxxx"
```
...
获取验证码 /auth/captcha?appid=xxx&captchaid=yyy

获取 token /auth/authenticate appid captchaid code

auth/router.authenticate

getTmsClient 可扩展

context/app.localCreateTmsClient

tms-koa-account/ checkCaptcha(ctx)
