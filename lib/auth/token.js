...
class TokenInRedis {
  ...
  static create() {
    return new Promise(resolve => {
      const redisConfig = require(process.cwd() + '/config/redis.js')
  ...
  scan(clientId) {
    return new Promise((resolve, reject) => {
      this.redisClient.scan('0', 'MATCH', `${INS_ID}:AccessToken:*:${clientId}`, (err, res) => {
        if (err) reject(err)
        else resolve(res[1])
      })
    })
  ...
  get(token) {
    return new Promise((resolve, reject) => {
      this.redisClient.scan('0', 'MATCH', `${INS_ID}:AccessToken:${token}:*`, (err, res) => {
...
class Token {
   ...
   */
  static supported() {
    return fs.existsSync(process.cwd() + '/config/redis.js')
  }
}
