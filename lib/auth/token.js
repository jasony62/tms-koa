...
const AppContextIns = AppContext.insSync()
const { auth: authConfig } = AppContextIns
if (!authConfig) {
      ...
      this.redisClient.scan(
        ...
        (err, res) => {
          if (err) return reject(err)
            ...
            if (res[0] !== "0") {
              return this.scan(clientId, res[0], keys).then(r => resolve(r)).catch(e => reject(e))
            } else {
              return resolve(keys)
            }
          }
      ...
      this.redisClient.scan(
        ...
        (err, res) => {
          if (err) return reject('access token error: redis error')
            ...
            if (res[1].length) keys.push(...res[1])
            if (res[0] !== "0") {
              return this.get(token, res[0], keys).then(r => resolve(r)).catch(e => reject(e))
            } else {
              if (keys.length === 1) {
                let key = keys[0]
...
class Token {
  ...
  static async create(tmsClient) {
    ...
    //是否支持多点登录
    if (AppContextIns.checkClientMultiLogin) {
      if ((await AppContextIns.checkClientMultiLogin(tmsClient)) === true) {
...
async function multiLogin(keys, tmsClient, tokenRedis) {
  ...
  if (expireIn < 1) return [false]
  // 解析出 token
  const reg = new RegExp('(?<=' + `${INS_ID}:AccessToken:` + ').*?(?=' + `:${tmsClient.id}` + ')')
