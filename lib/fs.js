 ...
 * @param {*} lfsConfig
 */
function initDir(instance, lfsConfig) {
  // 本地文件存储起始位置
  ...
  if (lfsConfig.domain && typeof lfsConfig.domain === 'object') {
    ...
    if (Array.isArray(lfsDomain.valid) && lfsDomain.valid.length) {
      if (lfsDomain.valid.indexOf(instance.defaultDomain) === 1)
        domains.splice(0, 0, instance.defaultDomain)
    } else {
...
class Context {
   ...
   * @param {*} request
   */
  async getClientBucket(client, domain, request) {
    if (!this.bucketHandler) return ''

    const bucket = await this.bucketHandler(client, domain, request)
