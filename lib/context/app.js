 ...
 * @param {*} client
 */
async function checkClientMultiLogin(client) {
  if (!this.multiLoginValidator) {
    logger.debug("未找到多点登录检测函数")
    return false
  }

  const rst = await this.multiLoginValidator(client)
  if (typeof rst !== "boolean") {
    logger.debug(`多点登录检测函数返回 ${rst} 格式错误, 非布尔型`)
    return false
  }
...
async function initAuth(instance, appConfig) {
  ...
  const { captcha, client, jwt, redis, bucket, accessToken } = auth
  const authConfig = {}
  ...
  if (authConfig.mode) {
    if (client && typeof client === 'object') {
      ...
      if (typeof path === 'string') {
        ...
        const fnCreateTmsClient = require(pathClient)
  ...
  if (accessToken && accessToken.multiLoginValidator && typeof accessToken.multiLoginValidator === 'string') {
    let validatorPath = modPath.resolve(accessToken.multiLoginValidator)
    try {
      const validator = require(validatorPath)
      if (typeof validator === 'function') {
        instance.multiLoginValidator = validator
        instance.checkClientMultiLogin = checkClientMultiLogin
        logger.info('指定多点登录检测函数')
      } else {
        logger.warn(`指定的多点登录检测路径[${accessToken.multiLoginValidator}]返回的不是函数`)
      }
    } catch (e) {
      logger.warn(`指定的多点登录检测路径[${accessToken.multiLoginValidator}]不存在`)
    }
  }
