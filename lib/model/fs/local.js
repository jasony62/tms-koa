...
class LocalFS {
  ...
  list(dir = '') {
    ...
    names.forEach(name => {
      ...
      if (stats.isFile()) {
        let relativePath = path.join(fullpath, name)
        files.push({ name, size: stats.size, birthtime: stats.birthtimeMs, path: relativePath })
  ...
  writeStream(filename, file, isRelative = true) {
    ...
    // 创建可写流
    const upStream = fs.createWriteStream(fullpath)
    // 可读流通过管道写入可写流
