...
class LocalFS {
  ...
  publicPath(fullpath) {
    ...
    /* 如果开放了文件下载服务添加前缀 */
    const { appConfig } = require('../../app').Context
    if (
      appConfig.router &&
      appConfig.router.fsdomain &&
      appConfig.router.fsdomain.prefix
    ) {
      publicPath = path.join('/', appConfig.router.fsdomain.prefix, publicPath)
    }
