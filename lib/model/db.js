...
class WhereAssembler {
  ...
  fieldNotBetween(field, match) {
    this.pieces.push(
      `${this.db.escapeId(field)} not between ${this.db.escape(match[0])} and ${this.db.escape(match[1])}`
    )
    return this
  }
      ...
      return new Promise((resolve, reject) => {
        conn.query(this.sql, (error, result) => {
          if (error) {
            ...
            if (isAutoIncId) resolve(result.insertId)
...
class SqlActionWithWhere extends SqlAction {
  ...
  get where() {
    if (!this.whereAssembler) this.whereAssembler = new WhereAssembler(this.db)
...
function getPool(oDbConfig) {
  // 只创建1次连接池
  if (cachedMasterDbPool && cachedWritableDbPool) return Promise.resolve([cachedMasterDbPool, cachedWritableDbPool])
...
function connect({ isWritableConn, conn: backupConn = null }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (cachedMasterDbPool === cachedWritableDbPool && isWritableConn && !backupConn) return backupConn
...
class DbContext {
  ...
  static getPool(pathOrConfig = process.cwd() + '/config/db.js') {
    ...
    if (typeof pathOrConfig === 'string') {
      if (!fs.existsSync(pathOrConfig)) return Promise.reject(`数据库连接配置文件(${pathOrConfig})不存在`)

      dbConfig = require(pathOrConfig)
    } else if (typeof pathOrConfig === 'object') dbConfig = pathOrConfig
    else return Promise.reject('没有指定数据库连接信息')

    if (typeof dbConfig.master !== 'object') return Promise.reject('没有指定默认数据库（master）连接参数')
   ...
   */
  static supported() {
    return fs.existsSync(process.cwd() + '/config/db.js')
  }
}
...
class Db {
  ...
  async conn(autoCreate = true) {
    let conn
    if (this.ctx.conn) conn = this.ctx.conn
    else if (autoCreate) {
  ...
  async writableConn(autoCreate = true) {
    let conn
    if (this.ctx.writableConn) conn = this.ctx.writableConn
    else if (autoCreate) {
