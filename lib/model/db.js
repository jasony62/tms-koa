...
function getPool(oDbConfig) {
    ...
    return new Promise((resolve, reject) => {
        dbPool.query('SELECT 1 + 1', function(error) {
            error ? reject(`建立默认数据库连接池（master）失败(${error.code})`) : resolve(dbPool)
        });
    }).then(dbPool => {
        ...
        return new Promise((resolve, reject) => {
            dbPool.query('SELECT 1 + 1', function(error) {
                error ? reject(`建立写数据库（write）连接池失败(${error.code})`) : resolve(dbPool)
            });
        }).then(dbPool => {
    ...
    return new Promise((resolve, reject) => {
        ...
        connPool.getConnection((err, conn) => {
            if (err) {
                ...
                reject(`获得${isWritableConn?'写':'默认'}数据库连接失败(${err.code})`)
            } else {
        ...
        return new Promise(resolve => {
            ...
            return cachedMasterDbPool ?
                new Promise(resolve => {
                    ...
                    console.log(`关闭默认数据库（Master）连接池`)
                }) :
                true
        }).then(() => {
            if (done && typeof done === 'function') done()
