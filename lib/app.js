...
class TmsKoa extends Koa {
  ...
  async fsInit(fsConfig) {
    ...
    if (fsSchemas && fsSchemas.some(s => typeof s.id !== 'string' || s.id.length === 0)) {
    ...
    try {
      ...
      stmt.where.fieldMatch('type', '=', 'table').fieldMatch('name', '=', fsDb.file_table)
      let tbl = await stmt.exec()
      if (!tbl) {
        let columns = ['id integer PRIMARY KEY autoincrement', 'userid text', 'path text']
        if (fsSchemas) {
          fsSchemas.forEach(s => {
        ...
        await dbContext.db().execSql(sqlCreateTable, true)
