...
class TmsKoa extends Koa {
  ...
  async startup({ beforeController, afterController } = {}) {
    ...
    if (appConfig.auth && appConfig.auth.disabled !== true) {
      ...
      if (typeof authConfig.jwt === 'object') {
        ...
        if (typeof host === 'string' && typeof port === 'number') {
          try {
            await require('./context/redis').Context.init({ host, port })
            mode = 'redis'
          } catch (e) {
    ...
    try {
      ...
      httpServer.listen(appConfig.port, (err) => {
        if (err) {
          logger.error(`启动http端口【${appConfig.port}】失败: `, err)
        } else {
          logger.info(`完成启动http端口：${appConfig.port}`)
        }
      })
    } catch (ex) {
      console.error('启动http服务失败\n', ex, ex && ex.stack)
    }
    /**
     * 支持https
     */
    if (typeof appConfig.https === 'object') {
