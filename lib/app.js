      ...
      if (fs.existsSync(fsConfigPath)) {
        ...
        if (fsConfig) {
          ...
          try {
            ...
            if (fsConfig.local) { // 自动创建文件夹
              if (typeof fsConfig.local.rootDir === "string" && !fs.existsSync(fsConfig.local.rootDir)) {
                fs.mkdirSync(fsConfig.local.rootDir + '/upload', {recursive: true})
                logger.info(`创建上传文件存储目录(${fsConfig.local.rootDir + '/upload'})`)
              }
              if (typeof fsConfig.local.outDir === "string" && !fs.existsSync(fsConfig.local.outDir)) {
                fs.mkdirSync(fsConfig.local.outDir, {recursive: true})
                logger.info(`创建系统生成文件存储目录(${fsConfig.local.outDir})`)
              }
            }
          } catch (e) {
