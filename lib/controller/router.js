...
async function fnCtrlWrapper(ctx, next) {
  ...
  try {
    ...
    let logMsg = `无法识别指定的请求，请检查输入的路径是否正确`
  ...
  let access_rule_white
  if (Object.prototype.hasOwnProperty.call(CtrlClass, 'get_access_rule')) {
    const access_rule = CtrlClass.get_access_rule()
    if (access_rule.rule_type === "white") access_rule_white = access_rule.actions
  }
  if (access_rule_white && access_rule_white.includes(method)) {
