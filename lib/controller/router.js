...
function findCtrlClassAndMethodName(ctx) {
  ...
  const npmCtrls = _.get(appConfig, 'router.controllers.plugins_npm')
  let npmCtrl
...
async function fnCtrlWrapper(ctx, next) {
  ...
  try {
    ...
    if (bucketValidateResult) {
      if (
        bucketValidateResult[0] !== true ||
        typeof bucketValidateResult[1] !== 'string'
      )
        return (response.body = new ResultFault('没有访问指定资源的权限'))
      oCtrl.bucket = bucketValidateResult[1]
    }
