...
async function fnCtrlWrapper(ctx, next) {
  ...
  if (typeof appConfig.auth === 'object' && appConfig.auth.jwt) {
    let token
    if (authorization && authorization.indexOf('Bearer') === 0) {
      token = authorization.match(/\S+$/)[0]
    } else if (request.query.access_token) {
      token = request.query.access_token
    } else {
      return (response.body = new ResultFault(
        '缺少Authorization头或access_token参数'
      ))
    }
    try {
      let decoded = jwt.verify(token, appConfig.auth.jwt.privateKey)
