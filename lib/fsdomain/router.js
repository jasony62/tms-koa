...
const { appConfig, FsContext } = require('../app').Context

let prefix = _.get(appConfig, ['router', 'fsdomain', 'prefix'], '')
  ...
  if (ctx.method === 'HEAD' || ctx.method === 'GET') {
    ...
    if (accessDomain) {
      try {
        ...
        if (ctx.request.query.attachment === "Y") ctx.attachment(filepath)
