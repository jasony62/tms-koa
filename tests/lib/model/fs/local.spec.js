...
describe('#model-file', function() {
  ...
  it('写文件', () => {
    ...
    let localFS = new LocalFS('lib/model/file', { fileConfig })
    ...
    expect(fs.writeFileSync.mock.calls[0][0]).toBe('tests/lib/model/file/tms-koa.txt')
  ...
  it('删文件', () => {
    ...
    expect(fs.removeSync.mock.calls[0][0]).toBe('tests/lib/model/file/tms-koa.txt')
  })
  it('文件列表', () => {
    const fileConfig = { local: { rootDir: 'tests' } }
    let localFS = new LocalFS('lib/model/fs', { fileConfig })
    localFS.list()
    expect(fs.readdirSync.mock.calls).toHaveLength(1)
    expect(fs.readdirSync.mock.calls[0][0]).toBe(path.resolve('tests/lib/model/fs'))
    expect(fs.readdirSync.mock.calls[0][1]).toMatchObject({ withFileTypes: true })
  })
})
