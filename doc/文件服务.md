...
在配置文件`config/fs.js`中指定`thumbnail`对象指定生成缩略图的参数。`dir`指定缩略图的存放位置，默认为：`_thumbs`，位于`domain`目录下。如果`domain`目录下文件的存储位置`/upload/202005/2208/30009144.png`，那么缩略图位于`_thumbs/upload/202005/2208/30009144.png`。`width`指定缩略图最大宽度，`height`指定缩略图最大高度。
