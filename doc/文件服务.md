...
配置文件`config/fs.js`中指定`rootDir`，`domain`和`bucket`。

上传文件的存储目录：`/rootDir/domain/bucket/YYYYMM/DDHH`。

上传文件的名称（通过表单上传），或自动命名（传递 base64 图片）：`mmss+4位随机数`。
...
通过`module`参数指定检查当前用户是否能够访问`bucket`方法的模块。
...
`tms-koa`支持直接下载`domain`中的文件，下载地址为`http://host:port/prefix/domain/filepath`。
