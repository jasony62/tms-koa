...
router.post('/register', (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        let [passed, userInfo] = yield registerTmsClient(ctx);
        if (passed === false) {
        ...
        logger.error(error);
        response.body = new ResultFault(error.message ? error.message : error.toString(), 20050);
    }
}));
router.get('/client', (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    let { response } = ctx;
    try {
        if (!authConfig.jwt && !authConfig.redis)
...
const authenticate = (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (!trusted[0]) {
            logger.warn(`有通过未授权主机调用auth::authenticate接口，原因：${trusted[1]}`);
            return (response.body = new ResultFault('不允许调用此接口'));
        }
        let [passed, tmsClient] = yield getTmsClient(ctx);
        if (passed === false) {
        ...
        if (authConfig.jwt) {
            ...
            const Token = require('./token');
            let aResult = yield Token.create(tmsClient);
            if (false === aResult[0]) {
...
const createCaptcha = (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    let { response } = ctx;
    try {
        let trusted = isTrustedHost(ctx);
        if (!trusted[0]) {
        ...
        if (typeof captchaConfig.createCaptcha === 'function') {
            let captcha = yield captchaConfig.createCaptcha(ctx);
            if (captcha[0] === false) {
        ...
        response.body = new ResultFault('未设置用验证码限制调用用户认证接口的方法', 20011);
    }
    catch (error) {
        logger.error(error);
        response.body = new ResultFault(error.message ? error.message : error.toString(), 20050);
...
const checkCaptcha = (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    let { response } = ctx;
    try {
        let trusted = isTrustedHost(ctx);
        if (!trusted[0]) {
        ...
        if (typeof captchaConfig.checkCaptcha === 'function') {
            let rst = yield captchaConfig.checkCaptcha(ctx);
            if (rst[0] === false) {
