...
function localCreateTmsClient(ctx, accounts) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (typeof checkCaptcha === 'function') {
            let cResult = yield checkCaptcha(ctx);
            if (cResult[0] === false)
                return cResult;
        }
        else
            return [false, '没有指定验证码检查程序'];
        let aResult;
...
function localRegisterTmsClient(ctx, accounts) {
    return __awaiter(this, void 0, void 0, function* () {
        const authConfig = Context.insSync().auth;
        let captchaConfig = _.get(authConfig, ['captcha'], {});
        const { checkCaptcha } = captchaConfig;
...
function checkClientBucket(ctx, client) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!this.bucketValidator)
            return [true, bucket ? bucket : ''];
        const result = yield this.bucketValidator(ctx, client);
        if (!Array.isArray(result))
            return [false];
        let [passed, validBucket] = result;
        if (passed !== true || typeof validBucket !== 'string')
            return [false];
...
function initServer(instance, appConfig) {
    ...
    if (https && typeof https === 'object') {
        ...
        if (valid)
            instance.https = { disabled, port, key, cert };
    }
    if (cors && typeof cors === 'object') {
        instance.cors = {};
        const { origin, credentials } = cors;
        if (origin && origin instanceof String)
            instance.cors.origin = origin;
        if (credentials === true)
            instance.cors.credentials = true;
    }
}
function initFsdomain(instanceRouter, fsdomainConfig) {
    ...
    if (Object.keys(validConfig).length)
        instanceRouter.fsdomain = validConfig;
}
function initRouter(instance, appConfig) {
    if (appConfig.router) {
        ...
        if (fsdomain)
            initFsdomain(instance.router, fsdomain);
    }
}
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (typeof jwt === 'object' && jwt.disabled !== true) {
            ...
            if (typeof privateKey === 'string') {
                ...
                authConfig.jwt = jwt;
                if (!expiresIn)
                    authConfig.jwt.expiresIn = 3600;
            }
            else
                logger.warn(`启用API调用认证机制[jwt]失败，参数不完整`);
        }
        else if (typeof redis === 'object' && redis.disabled !== true) {
            let { host } = redis;
            if (typeof host === 'string' || Array.isArray(host)) {
                try {
                    yield require('./redis').Context.init(redis);
                ...
                logger.warn(`启用API调用认证机制[redis]失败，参数不完整`);
        }
        if (authConfig.mode) {
            if (client && typeof client === 'object') {
                ...
                if (typeof npm === 'object' && npm.disabled !== true) {
                    ...
                    if (typeof id !== 'string')
                        throw Error(`通过[auth.client.npm.id]类型`);
        ...
        if (captcha && typeof captcha === 'object') {
            ...
            if (typeof npm === 'object' && npm.disabled !== true) {
                ...
                if (typeof id !== 'string')
                    throw Error(`通过[auth.captcha.npm.id]类型`);
...
class Context {
    ...
    static init(appConfig) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield initAuth(_instance, appConfig);
